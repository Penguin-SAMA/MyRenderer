...
void Model::store_materials() {
    ...
    for (int i = 0; i < this->mesh->polygons(); i++) {
        if (this->mesh->mat_list.size() - 1 >= i) {
            ...
            if (mat_p == NULL) {
                mat_p = this->materials["default"]; // 如果没有找到，就使用默认材质
            }
        } else {
            mat_p = this->materials["default"]; // 如果索引超出范围，就使用默认材质
        }

        // 如果多边形顶点大于4，将材质加入材质列表
...
void Model::store_triangles() {
    ...
    for (int i = 0; i < this->mesh->polygons(); i++) {
        face* fd0 = new face();
        if (this->mesh->f_list[i].size() > 4) { // 如果多边形顶点大于4，需要拆分为三角形
            for (int j = 1; j < mesh->f_list[i].size() - 2; j++) {
